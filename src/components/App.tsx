...
function App() {
  ...
  const submitPrompt = async () => {
    ...
    async function executeInterpreter(
      ...
      let data = {
        problem: HUMAN_EVAL_PROBLEMS[node.data.input]["prompt"],
      ...
      let url = "http://localhost:5175/execute";
      ...
      // let jsonResponse = {"result": {"passed": false, "result": "Error here"}}

      // Parse JSON response
      let jsonResponse = await response.json();
      console.log("json response", jsonResponse);

      const passed = Boolean(jsonResponse["result"]["passed"]);
      console.log("passed", passed);

      // var AUTO_EXPANSION = false;
      var AUTO_EXPANSION = settings.expandNodes;

      if (passed) {
        handleFinishedNode(node, true, false);
        AUTO_EXPANSION = false;
        setSettings({ ...settings, expandNodes: false });
        return null;
      } else {
        ...
        // let expand_rand = Boolean(Math.random() > 0.8);
        let expand_rand = true;

        if (!finalNode && AUTO_EXPANSION && expand_rand) {
          const promises = Array(settings.N_ANSWER_FANOUT)
                .fill(null)
                .map(async () => {
                  return await generateChild(node, "normal", "", true);
                });
          ...
          >
            ...
            >
              ...
              >
                ...
                <Box>
                  <p>In Toks: {inputTokenCount}</p>
                </Box>

                <Box>
                  <p>Out Toks: {outputTokenCount}</p>
                </Box>
